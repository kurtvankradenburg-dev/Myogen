...
export default function Auth({ navigate, setUser, googleAuthError, clearGoogleAuthError }) {
  ...
  async function handleGoogle() {
    ...
    try {
      ...
      // onAuthStateChanged in App.jsx detects the new user and navigates to dashboard
    } catch (err) {
      ...
      <div className="flex-1 flex flex-col justify-center px-6 py-12 lg:px-12">
        <div className="max-w-md w-full mx-auto">
          ...
          <button
            ...
            style={{ border: '1px solid rgba(255,255,255,0.2)', background: 'transparent', color: '#FAFAFA', cursor: googleLoading ? 'not-allowed' : 'pointer', opacity: googleLoading ? 0.7 : 1 }}
            onMouseEnter={e => { if (!googleLoading) e.currentTarget.style.background = 'rgba(255,255,255,0.05)'; }}
            onMouseLeave={e => e.currentTarget.style.background = 'transparent'}
          >
            {googleLoading ? (
              <div className="w-5 h-5 border-2 rounded-full animate-spin" style={{ borderColor: 'rgba(255,255,255,0.3)', borderTopColor: '#FAFAFA' }} />
            ) : (
